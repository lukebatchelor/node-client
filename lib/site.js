...
function deployFromZip(site, zip, draft) {
  ...
  return nodefn.call(fs.readFile, fullPath).then(function(zipData) {
    ...
    return site.client.request({
      ...
      /*******************************************************************
       * This is our hack to get our deployment URL.
       * We do an extra get request to ask for the latest deployments
       ******************************************************************/
      return site.client.request({
        url: path,
        type: 'get'
      }).then((res) => {
        // we get the last deploy id from our above request
        var lastDeployId = res.data[0].id;
        return new Deploy(site.client, {id: lastDeployId})
      });
    });
  });
};
